...
export function PurchaseOrderForm({ suppliers, warehouses, products, onSubmit }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(data) }} className="space-y-4">
      <div>
        ...
        {errors.supplier_id && <div className="text-red-500 text-xs">{errors.supplier_id}</div>}
      </div>
      <div>
      ...
      <div>
        ...
        {data.items.map((item, idx) => (
          <div key={idx} className="flex gap-2 items-center mb-2">
            ...
            <Input type="number" min={1} value={item.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)} className="w-24" />
            <Input type="number" min={0} value={item.unit_price} onChange={e => updateItem(idx, "unit_price", e.target.value)} className="w-24" />
      ...
      <Button type="submit" disabled={processing}>Submit Purchase Order</Button>
    </form>
  )
}
